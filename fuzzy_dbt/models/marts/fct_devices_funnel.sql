...
SELECT
  ...
  MAX(CASE WHEN p.pageview_url IN ('/the-original-mr-fuzzy',
                                    '/the-forever-love-bear',
                                    '/the-birthday-sugar-panda',
                                    '/the-hudson-river-mini-bear') THEN 1 ELSE 0 END) AS saw_product_detail,
  ...
  MAX(CASE WHEN p.pageview_url = '/thank-you-for-your-order'  THEN 1 ELSE 0 END) AS converted
FROM {{ ref('stg_website_sessions') }} s
JOIN {{ ref('stg_website_pageviews') }} p ON s.website_session_id = p.website_session_id
GROUP BY 1, 2
)
SELECT
  device_type,
  ...
  ROUND(SUM(converted)/SUM(saw_billing), 3) AS pct_to_order
FROM raw_funnel
GROUP BY 1